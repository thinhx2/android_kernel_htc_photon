...
static int spi_bma150_read_temp(void)
{
	char buffer[2];
	int ret;
...
static void bma150_early_suspend(struct early_suspend *handler)
{
	...
	if (!atomic_read(&PhoneOn_flag)) {
		...
		D("bma150_early_suspend: PhoneOn_flag is set\n");
	/*printk(KERN_DEBUG
...
static void bma150_early_resume(struct early_suspend *handler)
{
	...
	if (!atomic_read(&PhoneOn_flag)) {
		...
		printk(KERN_DEBUG "bma150_early_resume: PhoneOn_flag is set\n");
	/*printk(KERN_DEBUG
		"%s: spi_bma150_set_mode returned = %d!\n",
			__func__, ret);*/
}
...
static int  spi_bma150_probe(struct platform_device *pdev)
{
	...
	if (this_pdata->idev) {
		...
		if (!input_register_device(this_pdata->idev)) {
			printk( MODULE_NAME ": Input device successfuly registered\n");
		} else {
			this_pdata->idev = 0;
			printk(KERN_ERR MODULE_NAME ": Failed to register input device\n");
		}
	}
